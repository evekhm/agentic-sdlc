...
### [PR #451](https://github.com/evekhm/agentic-sdlc/pull/451): README Opens With the Use Case, Names the Orchestrator, and Explains the Living Spec
Adds a "What it solves" section at the front of README.md: the ladder as a harness-agnostic process whose every stage yields an artifact a person and the next agent both read, the team the owner composes and pins through `config/deployments.yaml` and the `DEPLOYMENTS` variable, what the loop does on its own, the two ways of working (at the keyboard with `/idea` then `/work <n>` one stage at a time, or handing off with `/work <n> --yolo` and the advisor's batch, with escalation as the only callback), and the seven doors in one list. `/next` ([#441](https://github.com/evekhm/agentic-sdlc/issues/441)) is withdrawn as a separate command before it was ever built; its scope becomes `/work <n>` without `--yolo`, and `<n>` itself becomes optional, resolved from the current worktree or offered as a pick from the operator's open issues. Adds `/claim <n>` and `/release <n>` as doors: they wrap `claim.sh` ([#87](https://github.com/evekhm/agentic-sdlc/issues/87)) and `claim.sh --release`, then move the session's working directory into the claimed worktree or back to the primary checkout, closing the gap `claim.sh` alone left (it names the worktree but never enters it). Adds "The living spec" as its own section with the deterministic guardrails behind it (spec check, changelog check, drift check, reviewers reading entries as claims). Rewrites "The orchestrator" around the advisor seat that guides the flow, watches for slips and files what it discovers ([#452](https://github.com/evekhm/agentic-sdlc/issues/452)), with the deterministic workflow beneath it and the per-issue `yolo`/`auto-close` labels plus the `loop.autonomy_enabled` kill switch from [#439](https://github.com/evekhm/agentic-sdlc/issues/439). Softens the reviewer model-family split to the suggested composition, removes the pin example block in favour of the config file, lists the two harnesses with one install footnote, and cuts "Running it yourself" to a placeholder for a forthcoming step-by-step guide. Readers arriving at the repository previously met the actors and the rungs before any statement of the problem or of how an operator works with the system. The section list and the affected decisions in `intent/35-readme/spec.md` are amended in the same change.
