...
Adds a "What it solves" section at the front of README.md: the ladder as a harness-agnostic process whose every stage yields an artifact a person and the next agent both read, the team the owner composes and pins through `config/deployments.yaml` and the `DEPLOYMENTS` variable, what the loop does on its own, and the two ways of working (at the keyboard with `/idea` and `/next`, or handing off with `/work <n> --yolo` and the advisor's batch, with escalation as the only callback). Readers arriving at the repository previously met the actors and the rungs before any statement of the problem or of how an operator works with the system. The section list in `intent/35-readme/spec.md` records the new section.
