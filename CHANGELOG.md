...
### Gemini Flash and Pro Token Pricing Pinned to 3.x Versions in Metering and Dispatch ([#269](https://github.com/evekhm/agentic-sdlc/issues/269))
`scripts/ops/session_spend.sh` and `scripts/ops/work.sh` priced all Gemini models at legacy Gemini 1.5 rates ($0.15 base input, $0.60 output per MTok for Flash; $1.25 base input, $5.00 output per MTok for Pro). Under these legacy rates, session spend was under-reported by ~5x on input and ~6.25x on output, and Antigravity dispatches could bypass configured `WORK_MAX_USD` ceilings. Both scripts now price modern Gemini Flash 3.x models (`3.5`, `3.6`, `3.7`, `3.8`) at $0.75 base input, $0.00 cache write, $0.075 cache read, and $3.75 output per MTok, and Gemini Pro 3.1 at $2.00 base input, $0.00 cache write, $0.20 cache read, and $12.00 output per MTok, matching Google Cloud Gemini Enterprise platform pricing and the repository cost schedule in `README.md`. Legacy 1.5 and 2.x rates are preserved for historical transcripts, and unrecognized models continue to fail closed.

### The Statusline Says Which Checkout a Session Is Driving ([#529](https://github.com/evekhm/agentic-sdlc/issues/529))
`scripts/ops/harness/statusline.sh` reported context against the 200K ceiling, spend, tokens, cache health, model and seat, and said nothing about where the session was working, so on a machine running several sessions at once — one worktree and one claim each — four terminals rendered four indistinguishable lines and the read-only primary checkout looked exactly like a claimed worktree. The line now ends with a location segment: `⑂ <branch>` in a `claim.sh` worktree whose directory name the branch already spells, `📂 <folder> ⑂ <branch>` in any other linked worktree, `📂 <folder> ⎇ <branch>` in a normal checkout, `📂 <folder>` outside a repository, and nothing when the payload carries neither a directory nor a branch. Both harnesses already point `statusLine.command` at this one script, so the segment reaches Claude Code and Antigravity with nothing new to install, and everything left of it — including the side-channel file the hooks read — is unchanged.

## 2026-09-17

### Test Suite Verdict Integrity and Subshell Accumulator Contract ([#405](https://github.com/evekhm/agentic-sdlc/issues/405))
Establishes mechanical test suite verdict integrity guarantees across `scripts/*/tests/*.sh` to prevent false-green test verdicts caused by subshell variable leakage or ambient environment corruption. Adds standalone test suite `scripts/ops/tests/suite_integrity_test.sh` implementing a null-implementation proof against stubbed runners, ambient-environment isolation proofs under hostile caller variables, and a static audit enforcing file-backed failure accumulators (`$FAIL_LOG`) for subshell tests. Self-sanitizes entry environments in `harness_test.sh`, `placement_test.sh`, and `wrap_test.sh`, isolates mutated runner executions via `env -u`, wires suite integrity verification into `.github/workflows/ci-gates.yml`, and codifies contract test standards in `AGENTS.md` and `docs/SPEC.md`.
...
### The Handover Contract Is Written Down, With Its Measured Cost ([#482](https://github.com/evekhm/agentic-sdlc/issues/482), [#483](https://github.com/evekhm/agentic-sdlc/issues/483), [#484](https://github.com/evekhm/agentic-sdlc/issues/484))
The property that makes this system harness-agnostic was load-bearing and unwritten: no agent communicates with another agent, and every handover is a write to GitHub followed by a cold read from GitHub by a process that starts later. Two agents are never alive at the same moment in the same handover, which is why a rung can move between vendors' binaries at all. AGENTS.md gains "The handover contract" as the binding form — the invariant, the three properties that follow (harness agnosticism, zero-model rung-to-rung transitions, restartability), a table of the six machine-readable contract resources with the sole writer of each, a MUST/MUST NOT list, and a named list of the four places the contract is not yet honored. README gains the same material as narrative for a person reading the repository, and `docs/SPEC.md` gains `process.handover` as the queryable current state. Three issues now track the gaps the writing exposed: the dispatch prompt carries no state and every cold start re-derives its rung by reading (#482), a fix round hands over a bare pull request number so the reviewer's findings reach the builder only because a model reads the comments (#483), and the ledger grammar is stated canonically in `scripts/ci/merge_gate.sh` and duplicated in `lifecycle_advance.sh` (#484). `docs/CRITICAL_PATH.md` tracks them as Gate H.

### Prompt Caching Is Documented as an Intra-Rung Economy
AGENTS.md's "Cost of execution" gains "Prompt caching does not cross a handover", written against a measured six-day window of poller-dispatched runs. The cache-read ratio tracks one variable, run duration — a 58-second run read 150K cached tokens at a 0.688 ratio while twenty-minute runs read 13–21M at 0.93 — which is the direct evidence that the prefix accumulates inside a single rung and is gone by the next cold start. Three operational consequences are now stated as rules: a harness switch at a rung boundary costs zero extra cache, so the hosting decision for a persona is free of cache considerations; buying a longer TTL to bridge a handover is always wasted, because nothing on the far side of a merge will read that prefix; and the only controllable number is the fresh input of the cold start, which is why #482 is the cost lever. The figures are dated in place and carry the caveat that they were measured on one harness — on the current pins every poller-dispatched rung is Antigravity, and the Claude seats run their reviewers from Actions — so they are re-measured before a rung persona moves to a different harness.
...
### Guided `/work` Claims and Posts as the Stage-Owning Persona ([#466](https://github.com/evekhm/agentic-sdlc/issues/466))
`work_dispatch.sh`'s unclaimed-issue branch called `claim.sh` with no `CLAIM_ACTOR`/`GH_TOKEN`, so the claim comment fell back to `git config user.name` and whatever `gh` login happened to be ambient — on an operator's own machine, their own admin login — instead of the persona owning that stage. `work_dispatch.sh` now resolves the stage the same way `claim.sh` derives it (a single `status:*` label, else `intent:new` names rung 1, else `implement` — `personas/lifecycle.json`), finds the owning persona the same way `work.sh`'s `owners_of()` does (`personas/*.yaml`, `kind: persona`, `stage: [...]`), mints that persona's App token via `scripts/auth/mint_app_token.py` (with `--require-repo`), and passes both through to `claim.sh`. Only stages with a single owner whose declared authority is not comment-only are claimed as that persona (reviewers never claim an issue or write labels; Argus R1-1 on PR #468). If no owner resolves, multiple owners exist, the owner is comment-only, or no token can be minted, it falls back to the previous unqualified call, so a misconfigured persona set degrades to prior behavior rather than refusing outright.

### `claim.sh` Fills In a Missing `status:*` Label ([#459](https://github.com/evekhm/agentic-sdlc/issues/459))
`scripts/ops/claim.sh` never wrote a `status:*` label. An issue that reached `claim.sh` with no `status:*` label at all — carrying only `intent:new`, or one a per-stage script never labeled — stayed unlabeled after the claim. `unattended.yml`'s review dispatch gates Argus and Atlas on `config/execution.yaml`'s `assigned_when.status_labels: [status:implementing]`, so a PR opened against such an issue silently got zero reviewers — the dispatch matrix showed `skipped`, with no error and no comment. `claim.sh` now resolves the label the same way `work.sh` resolves a rung: an issue with no `status:*` label but `intent:new` gets `personas/lifecycle.json`'s first rung (`status:planning`, with `intent:new` removed); an issue with neither is deliberately not on the ladder (every defect-repair issue) and gets no label at all. This is never driven by `CLAIM_STAGE` (or its literal default `implement`, which every documented caller falls back to) — an earlier cut of this fix defaulted to `implement` and force-promoted both of those states to `status:implementing`, corrupting real lifecycle state; Argus's round-2 review caught it. `blocked` now refuses the claim unconditionally, alongside the existing `hold` check, rather than only when the stage-label write happens to run; the label write's fail-closed re-read, immediately before writing, also now catches a `status:*` label that appeared on the issue since the claim began. An issue that already carries a `status:*` label — from a normal per-stage script, from `fast.sh`, or from a prior claim — is left exactly as it is, at any stage.
