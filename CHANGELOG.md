...
`scripts/ops/claim.sh` names a stage in its claim comment (`CLAIM_STAGE`, or the literal default `implement` — no documented caller sets `CLAIM_STAGE` today) but never wrote a matching `status:*` label. An issue that reached `claim.sh` with no `status:*` label at all — past `intent:new`, or one a per-stage script never labeled — stayed unlabeled after the claim. `unattended.yml`'s review dispatch gates Argus and Atlas on `config/execution.yaml`'s `assigned_when.status_labels: [status:implementing]`, so a PR opened against such an issue silently got zero reviewers — the dispatch matrix showed `skipped`, with no error and no comment. `claim.sh` now writes the claimed stage's label (via `personas/lifecycle.json`, removing `intent:new` if present) only when the issue carries no `status:*` label yet, re-checking `hold` and `blocked` immediately before the write; an issue that already carries a `status:*` label — from a normal per-stage script, from `fast.sh`, or from a prior claim — is left exactly as it is, at any stage.
