...
This harness has no native tool for the capabilities below. The
instruction under each is generated from config/tools.yaml — it is the
substitute procedure, and it is binding.

### ask_user

No interactive question tool is available on this harness. Write the
question out in full — the options, and the concrete case where they
differ — post it where the workflow's human gate will read it, and stop;
