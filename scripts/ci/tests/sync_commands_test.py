...
class SyncCommandsContractTest(unittest.TestCase):
    ...
    def test_d2_at_416_2_claude_targets_byte_identity(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Byte-identity against committed targets (drift check)
            for cmd in ["work.md", "idea.md", "bug.md"]:
                ...
                if not committed_file.is_file():
                    self.fail(f"D2 / AT-416-2: Committed target '{committed_file}' missing")
                ...
                if emitted_text != baseline_text:
                    self.fail(f"D2 / AT-416-2: Emitted .claude/commands/{cmd} has drifted from committed target")
