#!/usr/bin/env bash
# scripts/ops/tracker_search.sh --files <path>... [--terms <term>...]
#
...
for arg in "$@"; do
    ...
    case "$mode" in
        files) FILES+=("$arg") ;;
        terms) TERMS+=("$arg") ;;
        *) echo "usage: $0 --files <path>... [--terms <term>...]" >&2; exit 1 ;;
    esac
done
[ "${#FILES[@]}" -gt 0 ] || { echo "usage: $0 --files <path>... [--terms <term>...]" >&2; exit 1; }
...
echo "tracker searched, no prior art: files=${FILES[*]}${TERMS:+ terms=${TERMS[*]}}"
exit 0
