...
| Script | What it does | Invocation | Touches |
|---|---|---|---|
| `compiler_roundtrip.sh` | Six-step proof for the compiler: schema check, determinism (two builds byte-identical), no drift against committed targets, roundtrip re-parse, a throwaway persona compiles end to end, and a source carrying a home path is refused. | `bash scripts/ci/compiler_roundtrip.sh` | Temp directories only. |
