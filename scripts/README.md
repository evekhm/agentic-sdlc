...
| `harness/` | Context statusline, session priming, and handoff resolution suite (#330). Includes `install.sh` (wires dual-harness settings), `statusline.sh` (renders context against 200K ceiling, spend, and token sums; writes atomic side-channel state), `session-start.sh` (primes sessions with handoffs), `newest-handoff.sh`, and `newest-dated.sh`. Install and activation steps: [`ops/harness/README.md`](ops/harness/README.md). | `scripts/ops/harness/install.sh [--check|--uninstall]`; hooks called automatically by harness. | User and project settings files; `$CTX_DIR/<session_id>.json`. |
...
`scripts/ops/tests/harness_test.sh`.
