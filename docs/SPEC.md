...
The operator executes the eight-step enablement checklist prior to
...
2. Configure VM supervisor:
   ...
   Early supervisor startup is safe: the poller idles its queues when `loop.autonomous_merge` is false, and first-hop intake requires explicit opt-in via `intake:auto` capped by `loop.max_concurrent_first_hops` (PR #327, D1, D3, D4).
...
7. Provision labels:
   Run `scripts/setup/bootstrap_tracker.sh --labels-only` to ensure all
   labels, including `intake:auto`, are provisioned in the repository.
8. Initiate issue processing:
...
`loop.max_concurrent_first_hops` (PR #327, D1, D2, D3, D4). Live proofs before
...
### commands.compiler
`scripts/sync_commands.py` compiles canonical slash command sources in `commands/<name>.md` into per-harness targets (#416, `intent/416-commands-work-idea-bug/`): `.claude/commands/<name>.md` for Claude Code and `.agents/skills/<name>/SKILL.md` for Antigravity, all committed.

Format and Emitters:
- Claude Code targets (`.claude/commands/<name>.md`) preserve YAML frontmatter (`description`, `argument-hint`, `allowed-tools`) without marker comments, maintaining exact parser compatibility and byte-identity.
- Antigravity targets (`.agents/skills/<name>/SKILL.md`) carry the `# GENERATED by scripts/sync_commands.py` provenance marker and omit `allowed-tools` (which has no per-skill mapping in Antigravity).
...
`intake:auto` (opt-in for automated first-hop poller intake; PR #327, D4),
...
GraphQL payload shapes (`.author.login`) and REST API shapes (`.user.login`), while
first-hop concurrency checks in `scripts/placement/vm-local/poll.sh` normalize
both shapes by stripping any trailing `[bot]` suffix (PR #327, D3, D10). `.github/workflows/lifecycle.yml` writes the
...
- `max_concurrent_first_hops` (int): the positive integer fleet ceiling
  ...
  automated first-hop intake (PR #327, D3). Defaults to 1 if omitted.
...
idling notice and returns 0 without querying work queues (PR #327, D2).
...
- **Session location (#529):** Renders which checkout and branch the session is driving as the final segment, after model, effort and seat. A linked worktree renders `⑂ <branch>`, preceded by `📂 <folder>` unless the folder equals the branch with its slashes flattened (`<branch>` → `<branch//\//->`), which is exactly the `claim.sh` worktree naming (`.claude/worktrees/<actor>-<n>-<slug>` against branch `<actor>/<n>-<slug>`) and excludes a harness `agent-<hex>` tree, whose name is retained. A normal checkout renders `📂 <folder> ⎇ <branch>`; a directory outside any repository, or one on a detached HEAD, renders `📂 <folder>`; a payload carrying neither a directory (`workspace.current_dir`, falling back to `cwd`) nor `worktree.branch` renders no segment at all. The folder is the basename of the directory with any trailing slash removed. The branch is taken from `worktree.branch` when the harness supplies it, otherwise from a single `git rev-parse --absolute-git-dir --abbrev-ref HEAD` against the payload's directory, which also identifies a linked worktree by its git dir sitting under `/worktrees/`.
