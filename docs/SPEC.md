...
- **Session location (#529):** Renders which checkout and branch the session is driving as the final segment, after model, effort and seat. A linked worktree renders the branch alone marked `⑂ <branch>`, because the worktree directory is named after the branch; a normal checkout renders `📂 <folder> ⎇ <branch>`; a directory outside any repository renders `📂 <folder>`; a payload carrying neither a directory (`workspace.current_dir`, falling back to `cwd`) nor `worktree.branch` renders no segment at all. The branch is taken from `worktree.branch` when the harness supplies it, otherwise from a single `git rev-parse --absolute-git-dir --abbrev-ref HEAD` against the payload's directory, which also identifies a linked worktree by its git dir sitting under `/worktrees/`; a detached HEAD renders the folder alone.
