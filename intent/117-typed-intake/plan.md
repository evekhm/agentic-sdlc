...
2. In `.github/workflows/ci-gates.yml`: Add `scripts/ci/tests/intake_triage_test.sh` and `scripts/ci/tests/unattended_issues_test.sh` to the `execution` job run scripts (D13).
   - Environment prerequisite, and it is not optional: `unattended_issues_test.sh` resolves an `origin/main` baseline of `.github/workflows/unattended.yml` for the two unchanged-block comparisons (D8's `pull_request` trigger, D15's dispatch and file-level permission blocks). The `execution` job checks out with `actions/checkout@v4` and no `with:` block at all (`ci-gates.yml:87-88`), so it takes checkout's default `fetch-depth: 1` — a single commit, no `origin/main` remote-tracking ref, and `git show origin/main:...` exits 128. Give that job `with: fetch-depth: 0`, as the `spec-check` job in the same file already does (`ci-gates.yml:122-125`), or add an explicit `git fetch origin main` step before the suite. Without it the suite reports `FAIL: baseline origin/main unavailable` and the two comparisons do not run: the gate goes red by name rather than passing on a check nobody computed, but the fork-secrets surface those two comparisons guard is left unproven.
