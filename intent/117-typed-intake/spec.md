...
scripts/sync_agents.py                 load_lifecycle: terminal-ness over ladder rows only (D3)
scripts/ci/compiler_roundtrip.sh       step 7 gains two off-ladder refusal fixtures (D3)
...
| D3 | **A row is off-ladder exactly when its `label` is not a `status:*` label, and the file says so.** The `_comment` in `personas/lifecycle.json` gains: rows whose `label` is a `status:*` label are the ladder, in ladder order; a row whose label is not `status:*` is an off-ladder rung — a dispatcher may derive its stage from that label, the advancer never writes it, and it is appended after the ladder so the rank list is unchanged. The existing sentence "intake, deploy and maintain are stage-enum values with no rung and are absent by construction" becomes the same sentence about `intake` and `deploy`. `maintain` is already in `schema.json`'s stage enum and already declared by `personas/cassandra.yaml`. Accepted cosmetic consequence, stated so nobody edits the renderer to chase it: the renderer in `scripts/sync_agents.py` is not changed, so the generated block renders both `review` and `maintain` with "Last rung: nothing advances past it automatically" — each row's own bullets (its label, its artifact, its owner) already say which is which. **The validator in `scripts/sync_agents.py` IS changed, in exactly one function, `load_lifecycle` (:210-325), to reconcile with #52** (PR #187, merged as 919ef1a; the validator is commit ef6ce4d). This overrules nothing: #52 landed after this spec's round 3 and had no off-ladder row to reason about. That validator decides terminal-ness by position — `is_last` is the last element of `stages`, which must carry `advances_on` and `advances_to` null while every earlier row must not — so appending the `maintain` row makes `review` non-terminal by index and the build refuses the committed file with `stages[4] non-terminal rung cannot have advances_to null`. After this change the validator applies the filter this row defines: terminal-ness is computed over the rows whose `label` starts with `status:`, so `review` stays the terminal rung; every off-ladder row must carry `artifact`, `advances_on` and `advances_to` all null; and an off-ladder row that precedes the last `status:*` row is refused with an error naming the row and its position. The position check is not cosmetic: D2's safety argument — the rank comparison in `work.sh` never reaches the row — rests on the row sitting after `status:in-review`, and the compiler is the one place that can refuse a misplaced row deterministically instead of leaving the invariant to convention (a validator that filtered by label alone would build that misplaced file). Every other rule of `load_lifecycle` — key set, stage enum, unique labels, the `artifact`/`advances_on` pairing, exactly one `merge` — is unchanged, and the new checks run after them so every message the round-trip suite already pins still fires first. |
...
- `bash scripts/ops/tests/work_test.sh` passes and is the suite that
  carries the five `work.sh` rows above. Its #129 block (:599-611)
  keeps `issue 140 open "bug"`, both `run 2` exit codes and the
  empty-writes check unmodified, and exactly three `has` strings
  change: :605 and :611 from `refused: cannot derive a stage for #140`
  to the D5(f) refusal (`argus does not own stage maintain` and `atlas
  does not own stage maintain`), and :606 from `no status:* label and
...
- Two consecutive builds are byte-identical, and `sync_agents.py` is
  changed in exactly one function, `load_lifecycle`, and its renderer is
  untouched (D3).
- With the sixth row committed, `scripts/ci/compiler_roundtrip.sh`
  step 7 (:225-377) passes with its twelve `assert_lifecycle_refused`
  fixtures unmodified — each addresses a row by literal index
  (`stages[0]`, `[1]`, `[3]`, `[4]`) and never the last element, so
  `review` at index 4 is still the rung the "last rung" messages fire
  for — and gains exactly two fixtures at the end of the block: an
  off-ladder row (`label` not `status:*`) whose `advances_to` is
  non-null is refused with an error naming the row and the field; the
  committed `maintain` row moved to sit before `status:in-review` is
  refused with an error naming the row and its position (D3).
- Building a copy of `personas/` whose `lifecycle.json` carries the
  committed six rows exits 0; the same copy with the `maintain` row
  inserted between `status:build` and `status:implementing` exits 1
  with the position error, and `work.sh`'s rank list is never consulted
  on the refused file (D2, D3).
...
- `docs/SPEC.md` carries an `intake.triage` entry and the five
  ...
  says "five rungs" alone; `ops.dispatch` says "Nine refusals" and
