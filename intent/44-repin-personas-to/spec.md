...
3. The D3 resolution is recorded as one `(name, harness, model,
   ...
   the families are pairwise distinct; each family is declared with the
   vendor's own published naming as its evidence and also appears in
   D2's comment above `constraints:`; and the diff introduces no
   prefix, substring or regex test over a model ID that yields a family
   (D3, amended r2).
...
5. `scripts/sync_agents.py --check` and
   ...
   `config/deployments.yaml` if any persona moved,
   `config/model_tiers.yaml` if any tier was re-bound, and exactly the
   compiled targets those two edits account for and nothing else — a
   moved persona accounting for its old-harness target deleted and its
   new-harness target added, a re-bound tier accounting for the
   model-bearing line changing in every target on that harness that
   binds that tier, sub-agent targets included (D5, amended r2).
