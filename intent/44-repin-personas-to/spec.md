...
5. `scripts/sync_agents.py --check` and
   ...
   targets included; a commit that
   moved no persona and re-bound no tier is outside this item (D5,
   amended r2).
...
- PR #135 reviews (Argus R1-1 to R1-5, R2-3) and commits `11b88aa` and
  `dada939` (#271) on `main` — the pure `REVIEW` re-binds whose D2
  comment refresh D5's corrected `--stat` rule accounts for (added 2026-09-08, in r2 before
  merge).
