...
> [!NOTE]
> Notation: Every marker in this document is written with bracketed notation (such as `[review-verdict:<reviewer>:<verdict>]` or `[reviewed-head:<sha>]`) where real comments emit HTML comment syntax. This ensures the document complies with D6 and cannot be parsed as live verdict input by existing unanchored recorder parsers.
...
- **Anchored Marker Parsing & Markdown Structure Exclusion (D1, D5):** Pre-processing comments to strip code fences, indented code blocks, and blockquotes, while anchoring verdict markers to line start (`^[[:space:]]*[marker]`).
...
- All verdict marker regular expressions in `review_recorder.py` are anchored to line start (`^[[:space:]]*` followed by opening HTML comment tag and marker keyword: `review-verdict`, `reviewed-head`, `run-id`, `round`, `finding`, `failure-scenario`, `review-verdict-end`).
...
- Refusal reason codes:
  ...
  - `missing-run-id`: Missing or non-numeric `run-id` marker (`review_recorder.py:199`).
...
- Rule `marker` flags unescaped live verdict markers in HTML comment form:
  - `[review-verdict:...]`
  - `[review-verdict-end]`
  - `[reviewed-head:...]`
  - `[run-id:...]`
  ...
  - `[loop-ledger-end]`
  - `[refused-verdict:...]`
  *(Referenced above in bracketed notation per D6; the scanner detects their unescaped HTML comment counterparts).*
...
- **Allowlist Scope & Enumeration Strategy (R1-2 finding resolution):**
  To avoid discovery dilemmas at implement time and ensure `sanitize_check.sh` exits 0 cleanly without violating the allowlist contract, exactly 40 paths are categorized and enumerated for `scripts/ci/sanitize_allowlist.txt` (covering the 37 tracked files holding live markers today plus 3 forward-looking scanner and test files):
  1. *Infrastructure & Testing Sources (12 files)*: Files implementing, parsing, or testing marker mechanics (including the 3 forward-looking scanner files `sanitize_check.sh`, `sanitize_allowlist.txt`, and `sanitize_check_test.sh`).
  ...
  3. *Historical Immutable Intent Artifacts (20 files)*: Historical intent/spec/plan artifacts created before #318 that cannot be modified post-merge.
  *Note on `intent/318-consensus-recorder/plan.md`:* In strict compliance with Decision D6, this plan artifact uses bracketed notation throughout, carrying 0 live markers, and therefore requires no allowlist exemption.
...
- **Actions:**
  ...
  2. Anchor all marker regular expressions to line start: `^[[:space:]]*` followed by HTML comment marker.
  3. In `extract_verdict_blocks(comments, pr_commits)`:
     - Map `comment.user.login` to expected reviewer.
     - Only search for opening marker for expected reviewer (bracketed notation: `[review-verdict:{expected_reviewer}:...]`).
...
- **Actions:**
  ...
  3. Add the enumerated 40 allowlist entries in `scripts/ci/sanitize_allowlist.txt` under rule `marker` (covering 37 existing files holding live markers plus 3 forward-looking scanner files; `plan.md` uses bracketed notation with 0 live markers):
     ...
     - Pre-#318 historical immutable intent artifacts (20 files):
...
## 7. Plan Deviation: Reconciliation of AT-318-4 with Security Finding R1-1@D3

During Round 1 review, Argus and Atlas identified a critical security vulnerability (`R1-1@D3`) in the literal application of Acceptance Criterion AT-318-4.

### The Vulnerability in Literal AT-318-4
AT-318-4 in `spec.md` states:
> *"A comment authored by an unauthorized login containing a line-anchored `[review-verdict:argus:clean]` block fails author validation, logs an attributed audit note, and emits `[refused-verdict:argus:<head>:author-mismatch]` into the consensus ledger block."*

If any unauthorized external commenter posting a fake review verdict block causes the recorder to inject a machine-readable `refused-verdict` marker into the consensus ledger block, an unauthenticated attacker could permanently deny PR merges by triggering the refusal check in `merge_gate.sh:293-294` (denial of service).

### Reconciliation (Unanimous Consensus of Argus & Atlas)
In accordance with Decision D2 (*"Comments authored by logins outside authorized reviewers are not parsed for review verdict blocks"*), the plan reconciles D2 and D3:
1. **Untrusted Non-Reviewer Commenters (D2, R1-1):** Comments from logins outside the authorized reviewer set (`evekhm-argus-app[bot]`, `evekhm-atlas-app[bot]`) are skipped for verdict block parsing and MUST NEVER emit machine-readable `[refused-verdict:...]` markers into the consensus ledger block. They may only emit text notes under `#### Notes` (or be ignored).
2. **Authorized Reviewer Identity Mismatches (D3, AT-318-4 Refined):** When a comment authored by an authorized reviewer identity attempts to post a verdict block claiming to be a different reviewer persona (e.g. `evekhm-atlas-app[bot]` posting an `argus` verdict marker), the recorder fails author validation, logs an attributed audit note, and emits `[refused-verdict:argus:<head>:author-mismatch]` into the consensus ledger block.

This Plan Deviation formally supersedes the literal wording of AT-318-4 to prevent PR denial-of-service, preserving the security integrity of the autonomous merge gate.
