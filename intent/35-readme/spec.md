...
1. **What it solves** — the ladder as a harness-agnostic process whose every
   ...
   with `/work <n> --yolo` and the advisor's batch), the per-issue
   autonomy labels and the repository-wide kill switch; the doors
   (`/idea`, `/bug`, `/next`, `/work`, `/fast`, `/wrap`), one line
   each with its issue.
...
- No fenced command block exists; the six doors are named inline in
  section 1, once each with an issue link; no fenced block contains
  `scripts/ops/work.sh` or `/wrap` (D5, amended 2026-09-11).
