...
Exactly these nine `##` sections, in this order, and no others (D2):

1. **The loop in one picture** — what this repo is, and the diagram.
2. **Before you start** — clone, the six App private keys, `gh`, `jq`.
3. **File the change** — and who may file (the intake rule, D9).
4. **Type the number** — the one command (D5).
5. **What happens at each gate** — the five rungs, one short
   paragraph each.
6. **What you merge** — the gate is the acceptance (D8).
7. **Review** — two reviewers, two model families, by hand today.
8. **When something is wrong** — the halt labels and the defect path.
...
| D2 | **The section list above is exhaustive and ordered.** Nine `##` headings, in that order, no others and none nested deeper than `###`. A tenth topic is a link from section 9, never a tenth section — this is the mechanism that stops the welcome document from becoming a fourth standard (AGENTS.md, "No document sprawl"). Testable: `grep -c '^## ' README.md` is 9 and the headings match the list byte for byte. |
...
| D4 | **At most 150 lines, at most one diagram, no section over 25 lines.** A walkthrough an attendee will not finish is a transcript with a filename. Testable: `wc -l < README.md` ≤ 150; exactly one fenced diagram block; the longest span between two `## ` headings is ≤ 25 lines. |
...
| D10 | **The defect path states only what is ratified, and links #32 for the rest.** README's section 8 carries the halt behaviour already in the spec: `hold` halts all automation absolutely while present, `blocked` stops and reports, more than one `status:*` is corrupted state, and `review:3` escalates to `status:review-stuck` (`lifecycle.labels`, `review.policy`). Which rung a defect repair re-enters at is open on #32; README links the issue and asserts nothing. A guessed route in the attendee's first document is worse than a named gap. Testable: section 8 makes no stage-entry claim for a repair and contains a link to #32. |
...
- `README.md` exists at the repository root; `grep -c '^## '` is 9 and
  the headings equal D2's list in order, with no `####` (D2).
- `wc -l < README.md` ≤ 150; exactly one fenced diagram block; no span
  between two `## ` headings exceeds 25 lines (D4).
...
- Section 8 links #32 and makes no claim about which stage a defect
  repair re-enters (D10).
