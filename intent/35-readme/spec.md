...
1. **What it solves** — the ladder as a harness-agnostic process whose every
   ...
   work (at the keyboard with `/idea` then `/work <n>` one stage at a
   time, or handing off with `/work <n> --yolo` and the advisor's
   batch), the per-issue autonomy labels and the repository-wide kill
   switch; the doors (`/idea`, `/bug`, `/claim`, `/release`, `/work`,
   `/fast`, `/wrap`), one line each with its issue.
...
5. **The living spec** — `docs/SPEC.md` as the queryable current
   state beside the immutable per-change triple, the upsert rule, the
   artifact layout diagram, and the deterministic guardrails behind
   it (spec check, changelog check, drift check, reviewers reading
   entries as claims), each pointing at its owner.
6. **The orchestrator** — the advisor seat that guides the flow,
   watches for slips and files what it discovers; the deterministic
   workflow beneath it (recorder, merge gate, ledger, lifecycle,
...
| D4 | **At most one diagram.** *Superseded in part 2026-09-03 by the product owner: the original 150-line ceiling and 25-line section limit are lifted; no line or section-length cap applies.* Original rationale, kept for the record: a walkthrough an attendee will not finish is a transcript with a filename. *Amended 2026-09-09 by the product owner: the one-diagram cap is lifted. The product owner asked for diagrams and bullet points as the readability tools of the file. A diagram is a fenced `text` block of ASCII, at most one per section, and each renders a flow the surrounding prose already states: the loop, the merge-to-dispatch chain, the pin layout.* Testable now: every fenced block except the command block (D5) is tagged `text` and no section carries more than one. |
| D5 | **Exactly one command appears as an instruction: `/work <n>`, typed inside a harness session.** *Amended 2026-09-08 by the product owner: the instruction is the harness command. The product owner does not run `scripts/ops/work.sh` by hand (#89). The script is the command's implementation (#43 D16), and README may name it only in that role.* That is the whole typed input for every stage of every item (#36 D7, D8), so the walkthrough teaches one thing and cannot drift as stages change. Harness launch lines are *output* of the command (#36 D10) and never appear in README. Setup commands are not instructions here either — README links `scripts/auth/README.md` (D11). Testable: exactly one fenced block whose only line is `/work <n>`; every other fenced block is a D4 diagram; no fenced block contains `scripts/ops/work.sh`. *(Count amended 2026-09-09 with D4.)* *Amended 2026-09-10 with the handoff paragraph: `/wrap` (#85 D8) is named inline once as the session's close-out and never as a fenced instruction; the one fenced command stays `/work <n>`. Testable now: `grep -c '/wrap' README.md` prints 1 and no fenced block contains `/wrap`.* *Amended 2026-09-11 by the product owner (PR #451): the fenced command block is withdrawn with the running section it lived in. The commands are named inline, once each, in the "What it solves" doors list (`/idea`, `/bug`, `/work` with its `--yolo`/`--auto-close` flags, `/fast`, `/wrap`), each with its issue; `/work <n>` may recur in that section's prose and diagram. Testable now: every fenced block is a `text` diagram; no fenced block contains `scripts/ops/work.sh` or `/wrap`.* *Amended 2026-09-11 later the same day: `/next` (#441) is withdrawn as a separate command before it was ever built. Its scope — one stage, in-session, blocking, hands back the pull request — becomes `/work <n>` with no `--yolo`; `--yolo` is what makes it unattended and multi-stage. `<n>` is optional: `/work` alone resolves it from the current worktree's branch (`claim.sh` already names the issue there) or, absent that, from the last issue this session touched; when neither applies it lists the operator's open, unclaimed candidates and asks which one. README states this in the present tense. Testable now: `grep -c '/next' README.md` is 0.* *Amended 2026-09-12: `/claim <n>` and `/release <n>` join the doors list. `claim.sh` (#87) already verifies, claims and computes the worktree, and prints its path; the operator's session stays put after that, which is the gap #87's own title promised to close. The command files (`.claude/commands/claim.md`, `release.md`) close it: they wrap `claim.sh` / `claim.sh --release`, capture its last output line, and `cd` the session's shell into it (claim) or back to the primary checkout (release). Doors count is seven. Testable now: README names `/claim` and `/release` inline in the doors list, each with an issue link.* *Amended 2026-09-15 by the implementer (odyssey, PR #462 R1-3, under the owner-authorized fast-track): the three amendments above dated 2026-09-11 and 2026-09-12 were drafted on PR #451 by the operator's session and reached this table through PR #462; this note records them accurately, corrected in two places against the shipped command. First, D5 governs README's text and nothing else. The command's behaviour (the two modes, the resolution order for a bare `/work`, the `hold`/`blocked` circuit breaker, the claim rule, the stop point) is decided in `intent/441-work-optional-issue-number/spec.md` D1 to D7, and every README sentence about `/work` is bound by that spec (#441 D7). Second, without `--yolo` the session at the keyboard drives the stage itself once the digest has printed: the command starts no persona and returns no summary, so README's "At the keyboard" paragraph, the two-ways diagram and the doors line describe the one behaviour #441 D1 states. Testable now: `grep -c '/next' README.md` is 0; no README sentence has `/work <n>` without `--yolo` starting the owning persona on its pinned harness or handing back a summary; the doors list names seven commands inline, each with an issue link.* |
...
| D11 | **Take-home setup stays out of README (#35 OQ1).** Section 2 names the three prerequisites in one sentence each — a clone, the six App private keys, `gh` and `jq` — and links `scripts/auth/README.md` and `scripts/setup/`. Those instructions are already written and exercised where the scripts live; a second copy at the root would drift the first time an App is re-registered, and D3 forbids it. Testable: README contains no App-registration or label-provisioning step, only links. *Amended 2026-09-11 by the product owner (PR #451): the running section is cut to a placeholder that will link a step-by-step setup and demo guide written as its own document. The per-machine prerequisites survive as one footnote on the harness list in "Two harnesses, two model families", linking `scripts/ci/install_harness.sh`, `scripts/setup/`, `scripts/auth/README.md` and `scripts/ops/harness/README.md`.* |
...
- `README.md` exists at the repository root; every `## ` heading
  ...
  2026-09-03).
- Every fenced block is a `text` diagram and no section carries more
...
- No fenced command block exists; the seven doors are named inline in
  section 1, once each with an issue link; `grep -c '/next' README.md`
  is 0; no fenced block contains `scripts/ops/work.sh` or `/wrap` (D5,
  amended 2026-09-11 three times and again 2026-09-15).
- Every README sentence about `/work` without `--yolo` describes the
  session driving the stage after the digest; none has it starting
  the owning persona on its pinned harness or handing back a summary
  (D5, amended 2026-09-15; behaviour owned by #441 D1 and D7).
...
- The orchestrator section states that anyone files and the system
  files its own issues (the advisor on what it discovers, the
  maintainer's watchers), that a reviewer's findings on the PR under
  review stay in that thread, and links AGENTS.md; the diagram states
  that every issue starts `intent:new` with no `status:*` (D9; carrier
  widened to the diagram 2026-09-08 by the product owner, #263 R1-3;
  carrier moved to the orchestrator 2026-09-11). (Section numbers in
  this list follow the eight-section list of 2026-09-08; the "What it
  solves" section added at the front 2026-09-11 shifts each of them by
  one, and "The living spec" added the same day shifts sections 4 to
  8 by one more.)
- The orchestrator section states all three clauses: merge is
  acceptance, a closed PR is a rejection not relitigated, and an edit
  at the gate is the decision (D8, amended 2026-09-11).
- Sections 2 and 5 describe two independent reviewers, name which
  family each reads on under the current pins, point at
  `config/deployments.yaml`, and state the different-family split as
  the suggested composition (D6; softened from a hard constraint
  2026-09-11 by the product owner, PR #451).
