...
| D10 | **The defect path states only what is ratified, and points at its owner for the rest.** *Amended 2026-09-03 by the product owner: README names no issue numbers or tracker URLs. Amended again the same day once #32 was ratified: the re-entry rule is now settled, so README states it in one sentence and links its normative home, INTENT.md "Defect repair".* README's section 11 carries the halt behaviour already in the spec: `hold` halts all automation absolutely while present, `blocked` stops and reports, more than one `status:*` is corrupted state, and `review:3` escalates to `status:review-stuck` (`lifecycle.labels`, `review.policy`). The ratified repair path (#32): a repair whose `docs/SPEC.md` entry is unchanged skips the intent/spec/plan triple — issue, fix PR with a regression check, review, human merge — and a spec-changing repair re-enters at plan. Original rationale, kept for the record: while the route was open, a guessed route in the attendee's first document was worse than a named gap. Testable: the defect section states both halves of the ratified rule, links INTENT.md, and README contains no `#<n>` issue reference and no tracker URL. |
...
- The defect section states the ratified repair path (spec-unchanged
  ...
  links INTENT.md, and cites no issue number (D10).
