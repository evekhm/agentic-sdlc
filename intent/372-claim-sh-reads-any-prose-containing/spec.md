...
4. `.claude/commands/idea.md` and `.claude/commands/bug.md` gain the
   ...
   declaration.
5. Six open issues that currently carry a genuine leading "Depends
   ...
   #31→#11, #399→#85 and #330, #148→#64 and #147, #147→#44, #44→#43.
...
- Native dependency links created via `gh api` for the six pairs in
...
| **D7** | **Migration to native links is a one-time operational step, executed before the implementation PR's merge.** For each of the six pairs (#408→#85, #31→#11, #399→#85 and #330, #148→#64 and #147, #147→#44, #44→#43), the implementing session runs the two-command sequence from D5 against the live repository and verifies via a read of `dependencies/blocked_by` that each link now shows `state: open`, before the PR removing body parsing is merged. | *Adversary analysis:* Two defensible readings: (1) merge the `claim.sh` change first and backfill links afterward; (2) backfill first, verify, then merge. Differing case: between merge and backfill, a session claims #408 while #85 is still open. Under Reading 1, nothing refuses the claim: neither the old (removed) body scan nor the new (not-yet-linked) native check catches it, and the dependency silently stops being enforced for a real window. Under Reading 2, the native link exists before the old scan is removed, so the two mechanisms overlap and enforcement never lapses. This list is current as of 2026-09-15 (Atlas's round-1 review of this spec caught a sixth pair, #44→#43, missing from an earlier draft); the implementing session re-checks it immediately before executing the backfill in case a newly filed issue added a seventh pair meanwhile. |
...
- **AT-372-9 (D7):** Before the implementation PR merges, `gh api repos/evekhm/agentic-sdlc/issues/<n>/dependencies/blocked_by` for each of `n` in `{408, 31, 399, 148, 147, 44}` includes every paired blocker from D7 with `state: open`.
