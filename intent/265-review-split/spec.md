...
trust-bearing paths, on an open `security` row, or upon an explicit `review:deep`
grant.
...
scripts/ops/post.sh                      # trusted label application behind hold re-read
...
- **AT-12 (Label provisioning script):** `grep -F 'ensure_label "review:deep" "5319E7"' scripts/setup/bootstrap_tracker.sh` outputs:
  ```text
  ensure_label "review:deep" "5319E7" "Deep-review grant: triggers a full Argus review round out-of-band"
  ```
- **AT-13 (Label taxonomy documentation):** `grep -A 1 '### `review:deep`' scripts/setup/issues/04-label-taxonomy.md` outputs:
  ```text
  ### `review:deep`
...
- **Merge gate alignment with consensus follows assignment.** `scripts/ci/merge_gate.sh` is merged on main (PR #257, commit d61875c). Updating conjuncts (3) and (11) to evaluate the assigned set dynamically is owned by an amendment issue to #64.
