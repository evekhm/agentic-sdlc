...
- **AT-291-4 (D4)** Run `bash scripts/ci/tests/review_recorder_test.sh -k test_high_failure_scenario_demotion`: passes with exit code 0, verifying that a `high` finding lacking a sibling `failure-scenario` marker is demoted to `normal` with audit note `[demoted from high: missing failure_scenario marker] on <id>`.
...
- **AT-291-12 (D2)** Run `bash scripts/ci/tests/review_recorder_test.sh -k test_non_issue_reference_not_held`: passes with exit code 0, verifying that references to non-issue numbers scraped from PR body or branch name are logged as notes and treated as not held rather than failing closed.
