...
.claude/**                               # compiled persona targets generated from review-protocol.md
.agents/**                               # compiled persona targets generated from review-protocol.md
...
- **AT-3 (D2, D3)** Run `bash scripts/ci/tests/review_recorder_test.sh -k test_provenance_validation`:
  ...
  comment time. After withdrawal, merge_gate.sh's ATLAS_HEAD equals the previous
  head and conjunct 3 decides by #64 D7.
...
- **AT-13 (D4, D5)** Run `bash scripts/ci/tests/review_recorder_test.sh -k test_merge_gate_round_trip`:
  ...
  `scripts/ci/merge_gate.sh:271` emitting `R1-1@D4:high:open:none` under
...
- **AT-18 (D4)** Run `bash scripts/ci/tests/review_recorder_test.sh -k test_wire_format_assigned`:
  ...
  `scripts/ci/tests/merge_gate_test.sh:236-243` containing `<!-- assigned:argus,atlas -->`
...
- **Serialized execution and job ordering.** The recorder executes inside
  ...
  protecting the workflow because, quoting `merge-gate.yml:6-15`, "a same-repo
  pull_request run executes the workflow file from the pull request's head, so
  nothing that can write or mint the merge actor's token may run on that
  trigger." Idempotency guards skip execution when `github.event.sender.login`
  is Themis, and skip `PATCH` requests when the rendered body matches current
  comment content.
...
goes to the odyssey rung on branch `odyssey/267-severity-tiered-merge-gate-review-md`
after the plan because the advancer recognises `<actor>/<n>-<folder slug>`, and
no secret or GitHub App changes are required.
