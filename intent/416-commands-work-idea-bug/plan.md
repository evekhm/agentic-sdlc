...
- **Sanitizer Module Parity (AT-R1-2, R1-2, R1-9):** `scripts/sync_commands.py` imports `SECRET_PATTERNS` directly from `scripts.sync_agents`, eliminating literal `$HOME` strings from the compiler source and keeping `scripts/ci/sanitize_allowlist.txt` strictly untouched per D13.
