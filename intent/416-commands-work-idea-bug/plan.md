...
- **Door Allowlist Synchronization (R1-6):** In addition to `wrap.md` (D9, D12), `claim.md`, `fast.md`, and `release.md` were merged into `.claude/commands/` on `origin/main` after #416's plan baseline. To strictly honor D13's manifest without deleting unmigrated doors, `sync_commands.py` allowlists them via `ALLOWLIST_CLAUDE_FILES` alongside `wrap.md`.
- **Step 9 Roundtrip Gate Reconciliation (R1-7):** Plan section P10 referred to Step 8 of `compiler_roundtrip.sh`, but Step 8 is YAML frontmatter validation from PR #425; commands roundtrip and drift detection is wired as Step 9.
- **Cross-Harness /work Execution Hardening (R1-3, R1-4):** Emitted `.agents/skills/work/SKILL.md` distinguishes guided mode (default, invoking `scripts/ops/work_dispatch.sh [<number>]` via `run_command` without shelling out to `work.sh`) from unattended mode (`--yolo`, invoking `HEADLESS=1 scripts/ops/work.sh <number> [--as <persona>]`). The compiler strictly refuses unhandled `exec` hooks and verifies that the `work` command's hook invokes `work_dispatch.sh`.
- **Frontmatter Roundtrip Validation (R1-5):** `emit_claude` parses emitted frontmatter and asserts byte-level and semantic parity with the parsed dictionary, failing closed with `BuildError` if any key or scalar is distorted.
- **Sanitizer Module Parity (AT-R1-2, R1-2, R1-9):** `scripts/sync_commands.py` imports `SECRET_PATTERNS` directly from `scripts.sync_agents`, eliminating literal user home path strings from the compiler source and keeping `scripts/ci/sanitize_allowlist.txt` strictly untouched per D13.
