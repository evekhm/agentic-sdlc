...
| D12 | **Scope Boundary, Ordering Dependency, and Migration with PR #406 (#85).** Issue #416 migrates only existing merged commands: `work`, `idea`, and `bug`. Authoring and delivering `/wrap` remains strictly scoped to issue #85 (`intent/85-session-close-out/`, PR #406). Whichever PR lands second on `main` performs the mechanical migration of `.claude/commands/wrap.md` into `commands/wrap.md` (and at that point removes `wrap.md` from D9's allowlist). If #416 lands first, D9's allowlist protects PR #406 from drift failures; if PR #406 lands first, #416 implement migrates `wrap.md` into `commands/wrap.md` byte-identically. | Prevents circular dependencies, broken gates, and scope creep across parallel feature tracks. |
...
- **AT-416-2 (D2, D7)** Run `python3 scripts/sync_commands.py`: emitted files `.claude/commands/work.md`, `.claude/commands/idea.md`, and `.claude/commands/bug.md` are byte-for-byte identical to the baseline versions on `main` (`git diff .claude/commands/` is empty at implementation head). Frontmatter scalars are emitted verbatim without `yaml.safe_dump` re-wrapping or unicode escaping.
...
- **AT-416-4 (D3, D5)** In `.agents/skills/work/SKILL.md`, verify that the generated body instructs the agent to validate that the argument is a numeric issue/PR number optionally followed by `--as <persona>`, explicitly forbids shell metacharacters, and instructs execution of `HEADLESS=1 scripts/ops/work.sh <number> [--as <persona>]` via `run_command`.
...
- **AT-416-12 (D11, D13)** Verify `docs/SPEC.md` contains an updated section under `### commands.compiler` describing canonical command sources, compilation targets, execution model derivation, and drift gate enforcement; verify `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` document slash command compilation and Antigravity workspace skill discovery.
...
5. *Tool Permissions Mapping & Claude Frontmatter Formatting:* Resolved by D2, D4, and D5. Verbatim scalar formatting without YAML reflow preserves byte-identity on Claude Code; Antigravity translates permissions to validated tool-calling instructions.
