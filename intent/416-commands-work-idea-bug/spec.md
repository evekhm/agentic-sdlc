...
| D9 | **Target Directory Ownership, Scoped Pruning, and Pre-Existing-Defect Allowlist.** The target directories `.claude/commands/` and `.agents/skills/` are managed by `scripts/sync_commands.py`. Pruning in `.claude/commands/` is strictly scoped to `*.md` files directly in `.claude/commands/` (excluding non-.md files and subdirectories). Pruning in `.agents/skills/` is scoped to directories matching skills created by canonical commands. **Allowlist:** `.claude/commands/wrap.md` is explicitly allowlisted: `sync_commands.py` will not prune it and `sync_commands.py --check` will not flag it as drift. PR #406 (#85 implement) merged `wrap.md` onto `main` before this design rung's own head, so the allowlist no longer guards a merge-order race; it now stays in place because `wrap.md`'s frontmatter carries the same pre-existing YAML-parsing defect as `bug.md` (D14), under D14's fallback terms. Under `--check`, any non-allowlisted extraneous or orphaned file in target directories triggers a drift error (exit 1). | Eliminates stale or orphaned commands while keeping a file this compiler cannot yet ingest from tripping the drift gate. |
...
| D12 | **Scope Boundary and Migration Ordering with PR #406 (#85).** Issue #416 migrates only existing merged commands: `work`, `idea`, and `bug`. Authoring and delivering `/wrap` remains strictly scoped to issue #85 (`intent/85-session-close-out/`, PR #406), whose committed plan (`intent/85-session-close-out/plan.md:124`) does not permit touching `commands/wrap.md` and cannot be widened by this spec. PR #406 merged first (`625266e`, ahead of this design rung's own head), so `.claude/commands/wrap.md` is already live on `main`. #416's implement rung does NOT migrate `wrap.md` on #406's behalf merely because it landed first: `wrap.md`'s frontmatter carries its own pre-existing YAML-parsing defect, the same class as D14's `bug.md` finding — migrating it verbatim fails AT-416-1's parity with D1, and rewording it to parse breaks byte-identity. #416's implement rung migrates `commands/wrap.md` only once that defect has its own landed prerequisite fix (D14's fallback pattern); until then `wrap.md` stays in D9's allowlist. No new issue is filed for `wrap.md`'s defect as part of this spec: #85's `plan.md` and D9's allowlist already anchor `wrap.md`'s eventual migration to issue #85, and a dedicated prerequisite fix can be filed separately if a reviewer asks for one. | Prevents circular dependencies, broken gates, and scope creep across parallel feature tracks, and stops an implementer from being handed an order ("migrate wrap.md") that the file's own defect makes impossible to satisfy. |
...
| D14 | **Pre-Existing `.claude/commands/*.md` Frontmatter Defects and Migration Prerequisites.** `.claude/commands/bug.md` on `main` today contains an unescaped `: ` inside its `argument-hint` plain scalar (line 3), which is not valid YAML and cannot satisfy D1's frontmatter validation: migrating it verbatim fails parsing (AT-416-1), and rewording it to parse breaks D2's byte-identity clause (AT-416-2). This defect is pre-existing and independent of #416's design — fixing `.claude/commands/bug.md` (quoting the `argument-hint` scalar) is out of Athena's `intent/**` authority for this design rung and is filed as #425, a separate, narrowly-scoped issue against the existing file, not part of #416's diff. #416's implement rung migrates `commands/bug.md` only once that prerequisite fix has landed on `main`, measuring AT-416-1/AT-416-2 byte-identity against the corrected content. If the prerequisite has not landed by the time #416 implements, the implement rung migrates `work.md` and `idea.md` and adds `bug.md` to D9's allowlist mechanism (same treatment as `wrap.md`) until the prerequisite merges, rather than blocking the whole PR on an unrelated pre-existing bug. `.claude/commands/wrap.md`, landed on `main` by PR #406 (#85) after this design rung's own head, carries the identical class of defect: its `argument-hint` scalar (`[<seat-or-slug>] [--snapshot]`) is read as two adjacent flow sequences, which fails to parse at all, and repairing it to a single flow sequence still types `argument-hint` as a list where D1 requires a string. `wrap.md`'s migration is #85's responsibility (D12, D13; out of scope for #416), not #425's, so #416 does not migrate it and files no separate prerequisite issue for it now — #85's own plan and D9's allowlist already anchor `wrap.md`'s eventual migration and fix, and a reviewer can ask for a dedicated issue if that proves insufficient. `wrap.md` stays in D9's allowlist under the same fallback terms as `bug.md`. | Keeps the D1/D2 contradiction visible as a Decision instead of leaving an implementer to invent a resolution mid-implementation (Argus R1-1); scopes the actual code fix outside this design rung's authority, consistent with D13. Extended after Argus's round-4 verification (R4-1) found the identical contradiction on `wrap.md` once PR #406 merged it onto `main`. |
...
- **AT-416-13 (D9, D12, D14)** Verify that creating a hand-authored `.claude/commands/wrap.md` does not cause `python3 scripts/sync_commands.py --check` to fail, and running `python3 scripts/sync_commands.py` does not prune it, satisfying D9's allowlist under D14's fallback terms until `wrap.md`'s own pre-existing YAML defect has a landed prerequisite fix.
...
- **Target Pruning and Parallel Feature Tracks:** If a user or parallel branch stores non-command files in `.claude/commands/`, automated pruning could delete them. Decision D9 strictly scopes pruning to `*.md` files directly under `.claude/commands/` and skill directories under `.agents/skills/`. PR #406 merged `wrap.md` onto `main` before this design rung's own head; D9 and D14 keep it allowlisted going forward because its frontmatter carries the same pre-existing YAML defect as `bug.md`, not because of a merge-order race with #406, which is already resolved.
...
6. *Parallel Feature Decoupling (/wrap & PR #406):* Resolved by D9, D12, D14, and AT-416-13. PR #406 merged first; `.claude/commands/wrap.md` stays allowlisted going forward because its frontmatter carries the same pre-existing YAML defect as `bug.md` (D14), not because of the now-resolved merge-order race.
