...
| D9 | **Layout and Structure of `INTENT.md` Amendments.** Deciding the open question from the intent: dated amendments to `INTENT.md` are appended under a trailing `## Amendments` section at the bottom of `INTENT.md`. Each amendment is an H3 heading formatted as `### YYYY-MM-DD: <Issue Title> (#<issue>)`, followed by declarative text stating what non-negotiable statement was amended, why, and citing the amending intent. Founding text above the `## Amendments` section is never edited in place. | Preserves historical immutability of original founding decisions while providing a clean, chronological audit trail of accepted modifications. |
| D10 | **Tracker Search Decision Pass (`scripts/ops/tracker_search.sh --decisions`).** Deciding the open question from the intent: `scripts/ops/tracker_search.sh` is enhanced with a `--decisions` option. The pass searches `intent/*/spec.md` files for decision rows matching the pattern `^\|[[:space:]]*D[0-9]+[[:space:]]*\|` (or containing `| Dn |`) that match the search query terms (case-insensitively). Matching rows are printed in standard compiler/grep format: `<file>:<line>: <matching row>`. If any matching decision is found, the script marks `found=1`, prints refusal notice, and exits with code 2. The pass can be invoked standalone (`scripts/ops/tracker_search.sh --decisions <terms...>`) or combined with `--files` and `--terms`. | Provides fast, deterministic lookup of prior decisions across all specifications; integrates into the existing refusal mechanism. |
| D11 | **Tracker Label Provisioning in `scripts/setup/bootstrap_tracker.sh`.** `scripts/setup/bootstrap_tracker.sh` is updated to provision the missing labels: (1) `duplicate` with color `CFD3D7` and description `Duplicate: closed as duplicate of an existing issue or thread`; and (2) `area:*` labels with color `C5DEF5`: `area:personas`, `area:ci`, `area:ops`, `area:docs`, and `area:harness`. Running with `--labels-only` idempotently provisions these labels without touching issues. | Fulfills the prerequisites assumed by AGENTS.md search instructions and #117 intake automation. |
...
- **AT-1 (D1, D2, D4, D5, D6, D7):** Running `python3 scripts/sync_agents.py --check` succeeds with exit 0 (drift gate green); compiled targets `.claude/agents/athena.md` and `.agents/agents/athena/instructions.md` contain the full text of `intake-protocol.md`, `product-coherence.md`, and rows 6-8 of `spec-adversary.md`. (Falsifying input: omitting a skill or altering text causes `sync_agents.py --check` to exit 1 with diff output).
...
- **AT-3 (D1, D3):** A pull request authored by `evekhm-athena-app[bot]` that modifies `README.md` or `INTENT.md` satisfies authority path constraints in `merge_gate.sh`, because both files are enumerated in `authority.paths` in `personas/athena.yaml`. (Falsifying input: removing `README.md` from `authority.paths` causes path validation to decline merge).
...
- **AT-5 (D10):** Executing `scripts/ops/tracker_search.sh --decisions <term>` with a non-matching term (e.g. `nonexistenttermxyz123`) outputs zero matching decision lines and does not trigger exit code 2. (Falsifying input: matching on non-decision lines or exiting 2 on empty results fails this check).
- **AT-6 (D11):** Executing `bash scripts/setup/bootstrap_tracker.sh --labels-only` ensures the `duplicate` label and `area:*` labels (`area:personas`, `area:ci`, `area:ops`, `area:docs`, `area:harness`) exist on the remote tracker; running `gh label list --repo evekhm/agentic-sdlc` confirms each label is present with its declared color and description. (Falsifying input: missing `duplicate` or any `area:*` label from `bootstrap_tracker.sh` causes the label check to fail).
- **AT-7 (D8, D9):** An `INTENT.md` amendment structured under a trailing `## Amendments` section as `### YYYY-MM-DD: <Title> (#<issue>)` merges upon review approval by the operator with a single reviewer, requiring no secondary countersignature. (Falsifying input: requiring multiple reviewers or editing founding text in place fails this check).
