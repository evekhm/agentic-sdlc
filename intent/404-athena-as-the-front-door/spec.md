...
- **AT-3 (D1, D3):** Running the merge-gate authority path check for persona athena against a change set of README.md and INTENT.md returns allow; the same check against a path outside intent/**, README.md and INTENT.md (for example scripts/ops/post.sh) returns decline. (Falsifying input: a change set containing scripts/ops/post.sh that returns allow).
...
- **AT-5 (D10):** Executing `scripts/ops/tracker_search.sh --decisions <term>` with a non-matching term (e.g. `nonexistenttermxyz123`) outputs zero matching decision lines and exits 0. (Falsifying input: matching on non-decision lines or exiting 2 on empty results fails this check).
...
- **AT-7 (D8, D9):** An INTENT.md amendment appears under a trailing "## Amendments" heading as a subsection titled "### YYYY-MM-DD: <Title> (#<issue>)", and the diff modifies no line above that heading. (Falsifying input: an in-place edit of a line above the heading).
